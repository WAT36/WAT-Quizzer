...
export default function AccuracyRateGraphPage() {
  ...
  const displayChart = () => {
    ...
    for (let i = 0; i < checked_rate.length; i++) {
      let annotation_i =
        String(
          Math.round(parseFloat(checked_rate[i].accuracy_rate) * 10) / 10
        ) +
        '% / ' +
      ...
      visualized_data.push([
        '(チェック済問題)',
        parseFloat(checked_rate[i].accuracy_rate),
        '#32CD32',
        annotation_i
      ])
    }
    ...
    for (let i = 0; i < category_rate.length; i++) {
      let annotation_i =
        String(
          Math.round(parseFloat(category_rate[i].accuracy_rate) * 10) / 10
        ) +
        '% / ' +
