...
test('Add 1 Quiz.', async () => {
  ...
  // 問題追加
  result = await QuizService.addQuiz(
    0,
    'addQuizテスト問題,addQuizテスト答え,addQuizテストカテゴリ,addQuizテスト画像'
  )
...
test('Get 1 Quiz.', async () => {
  ...
  result = await QuizService.addQuiz(
    0,
    'getQuizテスト問題,getQuizテスト答え,getQuizテストカテゴリ,getQuizテスト画像'
  )
...
test('Random Get 1 of 5 Quiz by category.', async () => {
  ...
  // カテゴリ
  result = await testCommon.updateCategoryOfQuiz(
    'カテゴリ1:カテゴリ2:カテゴリ3',
    0,
    1
  )
  ...
  result = await testCommon.updateCategoryOfQuiz(
    'カテゴリ10:カテゴリ11:カテゴリ12',
    0,
    4
  )
  result = await testCommon.updateCategoryOfQuiz(
    'カテゴリ13:カテゴリ14:カテゴリ15',
    0,
    5
  )
...
// 不正解登録
test('Registering Incorrect.', async () => {
  // まず全消し
  let result = await testCommon.deleteAllQuizOfFile(0)
  ...
  // 不正解登録
  result = await QuizService.incorrectRegister(0, 1)
  ...
  expect(data[0].answer).toBe('addQuizテスト答え1')
  expect(data[0].clear_count).toBe(0)
  expect(data[0].fail_count).toBe(1)
  expect(data[0].category).toBe('addQuizテストカテゴリ1')
  expect(data[0].img_file).toBe('addQuizテスト画像1')
  expect(data[0].checked).toBe(0)
  expect(data[0].deleted).toBe(0)
...
test('Add Quiz to deleted quiz.', async () => {
  ...
  result = await QuizService.addQuiz(
    0,
    '削除済突っ込みテスト問題,削除済突っ込みテスト答え,削除済突っ込みテストカテゴリ,削除済突っ込みテスト画像'
  )
...
test('Edit quiz.', async () => {
  ...
  // 問題削除(3番目)
  result = await QuizService.editQuiz(
    0,
    3,
    '編集テスト問題文',
    '編集テスト答え',
